...
const AdminUserReportsViewer = ({ userId, patientName, onClose }) => {
  const { getUserReports } = useContext(AdminContext);
  ...
  useEffect(() => {
    const fetchReports = async () => {
      setLoading(true);
      const fetchedReports = await getUserReports(userId);
      setReports(fetchedReports || []);
      setLoading(false);
    };

    if (userId) fetchReports();
  }, [userId, getUserReports]);
        ...
        <div className='flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700'>
          <div>
            ...
            <p className='text-sm text-gray-500 dark:text-gray-400'>
              {patientName || 'User'}'s uploaded medical records
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className='overflow-y-auto p-5' style={{ maxHeight: 'calc(80vh - 80px)' }}>
          {loading ? (
            ...
            <div className='text-center py-12'>
              ...
              <p className='text-sm text-gray-400 mt-1'>This user hasn't uploaded any medical records yet.</p>
            </div>
          ) : (
            <div className='space-y-3'>
              {reports.map(report => (
...
export default AdminUserReportsViewer;
